...
const AnocmUI = () => {
  ...
  const [chatSettings, setChatSettings] = useState<{minTTL: number, defaultTTL: number, maxTTL: number} | null>(null);
  const [showAddUser, setShowAddUser] = useState(false);
  const [chatMessageTTLs, setChatMessageTTLs] = useState<{[chatId: string]: number | null}>({});
  ...
  const addUserToChat = async (chatId: string, userId: string, adminId: string, adminToken: string) => {
    try {
    ...
    try {
      ...
      if (data.success && data.userData) {
        ...
        if (chat.chatMessages) {
          return Object.entries(chat.chatMessages)
            .map(([ts, entry]) => {
  ...
  function getTtlOptions(min: number, def: number, max: number) {
    const presets = [min, def, max, 300, 900, 3600, 21600, 86400, 604800, 2592000];
    return Array.from(
...
const refreshChats = async (): Promise<string[]> => {
  ...
  try {
    ...
    const data = await res.json() as { success: boolean; userData?: string[] | string };
    if (!data.success) {
      setChats([]);
      return [];
    }
 ...
 const handleOpenChatMenu = async () => {
  ...
  const handleSendMessage = async () => {
    ...
    if(result.success){
      ...
      if(selectedTTL != null){
        console.log(`[SEND] Nachricht gesendet mit TTL: ${selectedTTL} Sekunden`);
      }
    }
    else {
  ...
  const handleAddUserToChat = async () => {
    ...
    const result = await addUserToChat(selectedChatId, newChatUserId.trim(), currentUser.userId, currentUser.token);
...
>
  ...
  {showChatMenu && (
    <div className="absolute right-0 top-full mt-2 bg-white border border-gray-200 rounded-lg shadow-lg p-3 min-w-64 z-10">
      <div className="text-xs text-gray-500 mb-3 px-1 font-medium">Chat-Verwaltung</div>
      ...
      <div className="px-3 py-2 bg-gray-50 rounded-lg mb-3">
        <div className="flex items-center space-x-2 mb-1">
          ...
          <span className="text-sm font-medium text-gray-700">
            {chatSettings ? `TTL: ${formatTTL(chatSettings.defaultTTL)}` : 'TTL: Standard'}
          </span>
        </div>
        {chatSettings && (
          <div className="text-xs text-gray-500">
            Min: {formatTTL(chatSettings.minTTL)} - Max: {formatTTL(chatSettings.maxTTL)}
          </div>
        )}
      </div>
  ...
  <div className="mb-4">
    ...
    <select
  value={
    chatMessageTTLs[selectedChatId!] === undefined || chatMessageTTLs[selectedChatId!] === null
      ? ""
      : chatMessageTTLs[selectedChatId!]
  }
  onChange={e => {
    const ttlValue = e.target.value === "" ? null : Number(e.target.value);
...
>
  {/* Standard-Option */}
  <option value="">Standard ({formatTTL(chatSettings.defaultTTL)})</option>
  {/* Dynamische Optionen von Backend */}
  {getTtlOptions(chatSettings.minTTL, chatSettings.defaultTTL, chatSettings.maxTTL)
    ...
    <div className="text-xs text-gray-400 mt-1">
      ...
      <div className="border-t pt-3">
        <div className="flex items-center justify-between mb-3">
          <span className="text-sm font-medium text-gray-700">👥 Teilnehmer ({selectedChat?.chatUserList ? Object.keys(selectedChat.chatUserList).length : 0})</span>
          <button
            onClick={() => {
              setShowAddUser(true);
              setShowChatMenu(false);
        ...
        <div className="space-y-2 max-h-40 overflow-y-auto">
          {selectedChat?.chatUserList && Object.entries(selectedChat.chatUserList).length > 0 ? (
            Object.entries(selectedChat.chatUserList).map(([userId, username]) => (
              <div key={userId} className="flex items-center justify-between py-2 px-2 bg-gray-50 rounded-md hover:bg-gray-100 transition-colors">
                <div className="flex items-center space-x-2">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-medium ${getAvatarColor(username, false)}`}>
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-gray-500">{userId.slice(0, 8)}...</span>
                  </div>
                </div>
                {userId !== currentUser?.userId && (
                  <button
                    onClick={() => handleRemoveUserFromChat(userId)}
            ...
            <div className="text-gray-500 text-sm italic py-4 text-center">Keine Teilnehmer gefunden</div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-sm">
            ...
            <div className="space-y-4">
              ...
              */}
              ...
              {/* TTL Preset Auswahl */}
              <div>
                <label className="text-sm font-medium text-gray-700">🕐 Verschwindende Nachrichten:</label>
                <select
                  value={selectedTTLPreset}
                  onChange={(e) => setSelectedTTLPreset(parseInt(e.target.value))}
                ...
                <div className="text-xs text-gray-500 mt-1">
                  {selectedTTLPreset === 1 && "Min: 5min, Standard: 1h, Max: 6h"}
                  {selectedTTLPreset === 2 && "Min: 1h, Standard: 1Tag, Max: 1Woche"}
                  {selectedTTLPreset === 3 && "Min: 1Tag, Standard: 1Woche, Max: 1Monat"}
                </div>
              </div>
